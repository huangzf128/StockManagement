...
/* 受注情報に発送日が格納されているレコードの実在庫からの出庫処理(18:00実行のみ)※受注分 */
PRINT '●発送済レコード実在庫からの出庫処理(実在庫あり)';
UPDATE TR 
Set ACTQTY = ACTQTY - T.SubtractQTY, UPDDT = GETDATE() 
FROM dbo.T_ActStock AS TR 
INNER JOIN (
 Select T1.ITEMCD, Sum(T1.RESERVQTY) As SubtractQTY 
 From (Select CRTDT, ITEMCD, ORDERID, RESERVQTY From dbo.T_StockReserv Where DELFLG = 0 AND ORDERSTATUS != 5) As T1 
 Inner Join (Select Distinct ID From JPONDATA.dbo.ORDERDAT Where 発送日 Is Not Null And 発送日 <= GETDATE() And OrderDate > '2018/02/01' And OrderDate > DATEADD(day, -360, GETDATE())) As T2 
  On T1.ORDERID = T2.ID 
 Left Join dbo.T_OutOrder As T3  
  On T1.ORDERID = T3.ORDERID 
 Where T3.ORDERID Is Null 
 Group By T1.ITEMCD 
) AS T 
 ON TR.ITEMCD = T.ITEMCD And TR.LOCATIONCD = '0';
PRINT ' 更新件数=' + CAST(@@ROWCOUNT AS VARCHAR) + '件更新しました';
...
FROM dbo.T_ActStock AS TR 
INNER JOIN (
 SELECT ITEMCD, Sum(RESERVQTY) As SubtractQTY 
 FROM (Select * From dbo.T_StockReserv WHERE DELFLG = 0 AND ORDERSTATUS = 3) As StockReserv 
 Group By ITEMCD 
) AS T 
 ON TR.ITEMCD = T.ITEMCD And TR.LOCATIONCD = '0';
PRINT ' 更新件数=' + CAST(@@ROWCOUNT AS VARCHAR) + '件更新しました';
...
INNER JOIN (
 Select T1.ITEMCD, Sum(T1.RESERVQTY) As SubtractQTY 
 From (Select CRTDT, ITEMCD, RESERVQTY, ORDERCODE, TELNO From dbo.T_StockReserv Where DELFLG = 0 AND ORDERSTATUS = 5) As T1 
 Inner Join 
 (Select Distinct ORDERMSG.YAHOOID, ORDERDAT.メールアドレス 
  From (Select ID, メールアドレス From JPONDATA.dbo.ORDERDAT Where 発送日 Is Not Null And 発送日 <= GETDATE() And OrderDate > '2018/02/01' And OrderDate > DATEADD(day, -360, GETDATE())) As ORDERDAT 
  Inner Join (Select 受注ID, Max(メッセージ1) As YAHOOID From JPONDATA.dbo.MSメッセージ Group By 受注ID) As ORDERMSG 
   On ORDERDAT.ID = ORDERMSG.受注ID 
 ) As T2 
  On T1.ORDERCODE = T2.YAHOOID And T1.TELNO = T2.メールアドレス 
 Group By T1.ITEMCD 
) AS T 
 ON TR.ITEMCD = T.ITEMCD And TR.LOCATIONCD = '0';
PRINT ' 更新件数=' + CAST(@@ROWCOUNT AS VARCHAR) + '件更新しました';
...
/* 実在庫にレコードが存在しない商品の出庫処理(18:00実行のみ)※受注分 */
PRINT '●発送済レコード実在庫からの出庫処理(実在庫なし)';
Insert Into dbo.T_ActStock(ITEMCD, LOCATIONCD, ACTQTY, UPDDT) 
Select T.ITEMCD, '0', 0 - T.SubtractQTY, GETDATE() 
From ( 
 Select T1.ITEMCD, Sum(T1.RESERVQTY) As SubtractQTY 
 From (Select CRTDT, ITEMCD, ORDERID, RESERVQTY From dbo.T_StockReserv Where DELFLG = 0 AND ORDERSTATUS != 5) As T1 
 Inner Join (Select Distinct ID From JPONDATA.dbo.ORDERDAT Where 発送日 Is Not Null And 発送日 <= GETDATE() And OrderDate > '2018/02/01' And OrderDate > DATEADD(day, -360, GETDATE())) As T2 
...
Left Join (Select * From dbo.T_ActStock Where LOCATIONCD = '0') AS TR 
On T.ITEMCD = TR.ITEMCD 
Where TR.ITEMCD Is Null;
PRINT ' 更新件数=' + CAST(@@ROWCOUNT AS VARCHAR) + '件追加しました';
...
/* 実在庫からの出庫処理分の履歴テーブルへの追加(18:00実行のみ)※受注分 */
PRINT '●出庫処理分の履歴テーブルへの追加';
Insert Into dbo.T_StockHistory(CRTDT, ITEMCD, LOCATIONCD, IOKBN, QTY, REMARKS, UPDIP) 
Select GETDATE(), T1.ITEMCD, '0', 'O', Sum(T1.RESERVQTY) As SubtractQTY, '発送済自動出庫', 'SYSTEM' 
From (Select CRTDT, ITEMCD, ORDERID, RESERVQTY From dbo.T_StockReserv Where DELFLG = 0 AND ORDERSTATUS != 5) As T1 
Inner Join (Select Distinct ID From JPONDATA.dbo.ORDERDAT Where 発送日 Is Not Null And 発送日 <= GETDATE()) As T2 
 On T1.ORDERID = T2.ID 
 Left Join dbo.T_OutOrder As T3  
  On T1.ORDERID = T3.ORDERID 
 Where T3.ORDERID Is Null 
...
/* 代理店分 */
Insert Into dbo.T_StockHistory(CRTDT, ITEMCD, LOCATIONCD, IOKBN, QTY, REMARKS, UPDIP) 
Select DATEADD(second, 1, GETDATE()), ITEMCD, '0', 'O', Sum(RESERVQTY) As SubtractQTY, '代理店受注自動出庫', 'SYSTEM' 
FROM (Select * From dbo.T_StockReserv WHERE DELFLG = 0 AND ORDERSTATUS = 3) As StockReserv
Group By ITEMCD;
PRINT ' 更新件数=' + CAST(@@ROWCOUNT AS VARCHAR) + '件追加しました';
...
/* 受注情報に発送日が格納されているレコードの引当情報論理削除(18:00実行のみ)※受注分 */
PRINT '●発送済レコード引当情報論理削除';
UPDATE TR 
Set DELFLG = 1, UPDDT = GETDATE(), UPDIP = '192.168.1.101' 
FROM dbo.T_StockReserv AS TR 
INNER JOIN (
 Select T1.CRTDT, T1.ITEMCD, T1.ORDERID 
 From (Select CRTDT, ITEMCD, ORDERID From dbo.T_StockReserv Where DELFLG = 0 AND ORDERSTATUS != 5) As T1 
 Inner Join (Select Distinct ID From JPONDATA.dbo.ORDERDAT Where 発送日 Is Not Null And 発送日 <= GETDATE()) As T2 
  On T1.ORDERID = T2.ID 
 Left Join dbo.T_OutOrder As T3  
  On T1.ORDERID = T3.ORDERID 
...
/* 代理店分 */
UPDATE dbo.T_StockReserv 
Set DELFLG = 1, UPDDT = GETDATE(), UPDIP = '192.168.1.101' 
WHERE DELFLG = 0 AND ORDERSTATUS = 3;
PRINT ' 更新件数=' + CAST(@@ROWCOUNT AS VARCHAR) + '件更新しました';
...
INNER JOIN (
 Select T1.CRTDT, T1.ITEMCD, T1.ORDERID 
 From (Select CRTDT, ITEMCD, ORDERID, ORDERCODE, TELNO From dbo.T_StockReserv Where DELFLG = 0 AND ORDERSTATUS = 5) As T1 
 Inner Join 
 (Select Distinct ORDERMSG.YAHOOID, ORDERDAT.メールアドレス 
  From (Select ID, メールアドレス From JPONDATA.dbo.ORDERDAT Where 発送日 Is Not Null And 発送日 <= GETDATE() And OrderDate > '2018/02/01' And OrderDate > DATEADD(day, -360, GETDATE())) As ORDERDAT 
  Inner Join (Select 受注ID, Max(メッセージ1) As YAHOOID From JPONDATA.dbo.MSメッセージ Group By 受注ID) As ORDERMSG 
