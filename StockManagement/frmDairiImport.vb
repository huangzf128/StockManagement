Public Class frmDairiImport
    Private Sub cmdExec_Click(sender As Object, e As EventArgs) Handles cmdExec.Click
        ...
        Dim varItem(5) As Object
        Dim varValue As Object
        Dim varSplitVal As Object
        Dim varContent As Object
        ...
        If Trim(Cm.Nz(txtFilePath.Text, "")) <> "" Then
            ...
            If Len(strFileName) > 4 And strFileName.Substring(strFileName.Length - 3) = "csv" Then
                ...
                ' CSVファイルの取込み
                ' TODO
                'DoCmd.TransferText acImportDelim, "Imp_CSV2", "T_ImportCSV", txtFilePath.Text, True

                ' TODO
                ' レコードセットのセット
                'Set objDB = CurrentDb()
                'Set objRS = objDB.OpenRecordset("T_ImportCSV", dbOpenForwardOnly)
                '' CSV取込みテーブルを全件ループ
                'Do Until objRS.EOF
                '        varItem(0) = objRS![品名１]
                '        varItem(1) = objRS![品名２]
                ...
                '                                         "Values('" & Cm.Nz(objRS![ご依頼主名称１], "") & "', '" & strItem & "', " & strQty & ")"
                '                        Else
                '                            ' エラーテーブルに格納
